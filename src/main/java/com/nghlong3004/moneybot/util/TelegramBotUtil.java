...
public class TelegramBotUtil implements LongPollingSingleThreadUpdateConsumer {
  ...
  private void processTransactionEntry(MessageConfiguration messageModel) {
    ...
    String aiResponse = ObjectContainerUtil.getAiRequesterUtil().ask(prompt);
    AI result = parseJsonToAIModel(aiResponse);
    String periodOfDay =
        result.getPeriodOfDay().startsWith("kh") ? "" : result.getPeriodOfDay() + " ";
    Expense expense = Expense.builder().userId(messageModel.getChatID()).amount(result.getAmount())
        .type("THU".equals(result.getType()) ? "income" : "expense")
        .description(periodOfDay + result.getCategory())
        .updatedAt(new Timestamp(System.currentTimeMillis())).build();
    sendTextMessage(messageModel.getChatID(), result.getDate() + " " + expense.getAmount() + " " + expense.getDescription());
  }
  ...
  private AI parseJsonToAIModel(String jsonString) {
    ObjectMapper objectMapper = new ObjectMapper();
    objectMapper.setPropertyNamingStrategy(PropertyNamingStrategies.SNAKE_CASE);
    objectMapper.findAndRegisterModules();
    try {
      return objectMapper.readValue(jsonString, AI.class);
